...
+ (BOOL)point:(CGPoint)point inArea:(NSArray<NSValue *> *)area {
    ...
    if ([[XAuxiliaryCalculationHelper shareCalculationHelper] containPoint:[NSValue valueWithCGPoint:point] Points:[NSMutableArray arrayWithArray:area]]) {
        return YES;
    }
    else {
        return NO;
...
+ (NSArray<NSValue *> *)expandRectArea:(NSArray<NSValue *> *)area expandLength:(CGFloat)length {
    ...
    CGPoint rectPoint1 = CGPointMake(originP1.x - length/2, originP1.y - length/2);
    NSValue *value1 = [NSValue valueWithCGPoint:rectPoint1];
    CGPoint rectPoint2 = CGPointMake(originP2.x - length/2, originP2.y + length/2);
    NSValue *value2 = [NSValue valueWithCGPoint:rectPoint2];
    CGPoint rectPoint3 = CGPointMake(originP3.x + length/2, originP3.y - length/2);
    NSValue *value3 = [NSValue valueWithCGPoint:rectPoint3];
    CGPoint rectPoint4 = CGPointMake(originP4.x + length/2, originP4.y + length/2);
